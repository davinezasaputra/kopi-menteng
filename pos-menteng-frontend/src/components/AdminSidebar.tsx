...
const allowed = (i: MenuItem) => i.permission ? can(i.permission) : i.anyOf ? canAny(i.anyOf) : true;
...
const modules: ModuleGroup[] = [
 { key:'erp',label:'ERP',icon:'🏢',subgroups:[
  { key:'erp-overview',label:'Overview',items:[{key:'operations',label:'Operations Center',icon:'📊',path:'/erp/operations',anyOf:['inventory.stock.view','purchasing.supplier.view','purchasing.order.view','sales.order.view','accounting.report.view']}] },
  { key:'erp-inventory',label:'Inventory',items:[{key:'inventory',label:'Produk',icon:'📦',path:'/inventory',permission:'inventory.stock.view'},{key:'inventory-operations',label:'Kontrol Persediaan',icon:'📈',path:'/inventory/operations',anyOf:['inventory.stock.view','inventory.stock.adjust']},{key:'raw-materials',label:'Bahan Baku',icon:'🫙',path:'/raw-materials',permission:'inventory.stock.view'}] },
...
const filterModule=(m:ModuleGroup):ModuleGroup|null=>{const items=m.items?.filter(allowed)??[];const subgroups=m.subgroups?.map(g=>({...g,items:g.items.filter(allowed)})).filter(g=>g.items.length>0)??[];return items.length||subgroups.length?{...m,items,subgroups}:null;};
export default function AdminSidebar({activePage='dashboard'}:AdminSidebarProps){
 const navigate=useNavigate();
 const user=useMemo(()=>{try{return JSON.parse(localStorage.getItem('user')||'{}') as {name?:string};}catch{return {}; }},[]);
 ...
 const allowedModules=useMemo(()=>modules.map(filterModule).filter((m):m is ModuleGroup=>!!m),[]);
