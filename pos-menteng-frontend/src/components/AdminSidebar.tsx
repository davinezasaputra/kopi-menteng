...
type MenuItem = { key: NonNullable<AdminSidebarProps['activePage']>; label: string; icon: string; path: string; permission?: string; anyOf?: string[] };
...
const moduleGroups: ModuleGroup[] = [
  {
    ...
    subgroups: [
      ...
      { key: 'erp-purchasing', label: 'Purchasing', items: [
...
function filterModule(module: ModuleGroup): ModuleGroup | null {
  ...
  const subgroups = module.subgroups?.map(group => ({ ...group, items: group.items.filter(itemAllowed) })).filter(group => group.items.length > 0) ?? [];
  return items.length || subgroups.length ? { ...module, items, subgroups } : null;
}
...
export default function AdminSidebar({ activePage = 'dashboard' }: AdminSidebarProps) {
  ...
  const allowedModules = useMemo(() => moduleGroups.map(filterModule).filter((module): module is ModuleGroup => module !== null), []);
  const activeModule = useMemo(() => allowedModules.find(module =>
    (module.items ?? []).some(item => item.key === activePage) ||
    (module.subgroups ?? []).some(group => group.items.some(item => item.key === activePage)),
  )?.key ?? null, [activePage, allowedModules]);
  const activeSubGroup = useMemo(() => allowedModules.flatMap(module => module.subgroups ?? []).find(group => group.items.some(item => item.key === activePage))?.key ?? null, [activePage, allowedModules]);
  ...
  const handleLogout = async () => {
    ...
    try { await axios.post('/v1/auth/logout'); toast.success('Berhasil Logout!', { id: toastId }); }
      ...
      localStorage.removeItem('token'); localStorage.removeItem('user'); localStorage.removeItem('permissions'); localStorage.removeItem('erp_context'); localStorage.removeItem('foundation_loaded'); navigate('/');
    }
  };
  ...
  return (
    <aside className="w-72 bg-stone-900 text-stone-300 flex flex-col">
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        <button onClick={() => navigate('/dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition text-left ${activePage === 'dashboard' ? 'bg-amber-700/20 text-amber-500 font-medium' : 'hover:bg-stone-800 hover:text-white'}`}><span>📊</span>Dashboard</button>
        {allowedModules.map(module => {
          const isOpen = openModules.includes(module.key);
          const isActive = activeModule === module.key;
          return <div key={module.key}>
            <button onClick={() => setOpenModules(current => current.includes(module.key) ? current.filter(value => value !== module.key) : [...current, module.key])} className={`w-full flex items-center justify-between gap-3 px-4 py-3 rounded-xl transition text-left ${isActive ? 'bg-stone-800 text-white' : 'hover:bg-stone-800 hover:text-white'}`}>
              <span className="flex items-center gap-3"><span>{module.icon}</span><span className="font-semibold">{module.label}</span></span><span className="text-xs text-stone-500">{isOpen ? '⌃' : '⌄'}</span>
            </button>
            {isOpen && <div className="mt-1 ml-3 space-y-1 border-l border-stone-800 pl-2">
              {(module.items ?? []).map(item => <button key={item.key} onClick={() => navigate(item.path)} className={`w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm ${activePage === item.key ? 'bg-amber-700/20 text-amber-500 font-medium' : 'hover:bg-stone-800 hover:text-white'}`}><span>{item.icon}</span>{item.label}</button>)}
              {(module.subgroups ?? []).map(group => {
                const open = openSubGroups.includes(group.key);
                const active = group.items.some(item => item.key === activePage);
                return <div key={group.key}>
                  <button onClick={() => setOpenSubGroups(current => current.includes(group.key) ? current.filter(value => value !== group.key) : [...current, group.key])} className={`w-full flex items-center justify-between rounded-lg px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide ${active ? 'text-amber-500' : 'text-stone-500 hover:text-stone-300'}`}><span>{group.label}</span><span>{open ? '−' : '+'}</span></button>
                  {open && <div className="space-y-1 pl-2">{group.items.map(item => <button key={`${group.key}-${item.key}`} onClick={() => navigate(item.path)} className={`w-full flex items-center gap-3 rounded-lg px-3 py-2 text-left text-sm ${activePage === item.key ? 'bg-amber-700/20 text-amber-500 font-medium' : 'hover:bg-stone-800 hover:text-white'}`}><span>{item.icon}</span>{item.label}</button>)}</div>}
                </div>;
              })}
            </div>}
          </div>;
