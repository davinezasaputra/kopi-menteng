...
type TenantSummary = { id: number; code: string; name: string; status: string; timezone?: string; currency?: string; company_count: number; branch_count: number };
type TenantOrganization = TenantSummary & { companies: Company[] };
...
export default function DeveloperOrganization() {
  ...
  const companies = useMemo(() => organization?.companies ?? [], [organization]);

  const loadTenants = async () => {
    try {
      const response = await api.get('/v1/developer/tenants');
      setTenants(response.data?.data ?? []);
    } catch (error) { toast.error(errorMessage(error) || 'Daftar tenant gagal dimuat.'); }
  };

  const loadOrganization = async (id: string) => {
    if (!id) { setOrganization(null); return; }
    try {
      const response = await api.get(`/v1/developer/tenants/${id}/organization`);
      setOrganization(response.data?.data ?? null);
    } catch (error) { toast.error(errorMessage(error) || 'Hierarchy organisasi gagal dimuat.'); }
  };

  useEffect(() => { if (developer) void loadTenants(); }, [developer]);
  useEffect(() => { void loadOrganization(tenantId); setExpandedCompany(null); setExpandedBranch(null); }, [tenantId]);

  const refreshOrganization = async () => { await Promise.all([loadTenants(), loadOrganization(tenantId)]); };

  const addCompany = async () => {
    if (!tenantId || !companyForm.code || !companyForm.name) return toast.error('Tenant, code, dan nama company wajib diisi.');
    try { await api.post(`/v1/developer/tenants/${tenantId}/companies`, companyForm); toast.success('Company berhasil dibuat.'); setCompanyForm({ ...emptyCompany }); await refreshOrganization(); }
  ...
  const addBranch = async (companyId: number) => {
    ...
    try { await api.post(`/v1/developer/tenants/${tenantId}/companies/${companyId}/branches`, branchForm); toast.success('Branch berhasil dibuat.'); setBranchForm({ ...emptyBranch }); await refreshOrganization(); }
  ...
  const addLocation = async (companyId: number, branchId: number) => {
    ...
    try { await api.post(`/v1/developer/tenants/${tenantId}/companies/${companyId}/branches/${branchId}/locations`, locationForm); toast.success('Location berhasil dibuat.'); setLocationForm({ ...emptyLocation }); await refreshOrganization(); }
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
          <div className="grid gap-4 md:grid-cols-[1fr_auto]">
            ...
            {organization && <div className="self-end rounded-xl bg-stone-900 px-4 py-2.5 text-sm font-bold text-white">{organization.company_count ?? companies.length} Company · {organization.branch_count ?? companies.reduce((n,c)=>n+c.branches.length,0)} Branch</div>}
          </div>
        </section>

        {organization && <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
          <div><h2 className="font-black">Companies</h2><p className="text-sm text-stone-500">Satu tenant dapat memiliki banyak legal/business company.</p></div>
          <div className="mt-4 grid gap-3 md:grid-cols-3"><Field label="Company Code" value={companyForm.code} onChange={v => setCompanyForm(f => ({ ...f, code: v }))} /><Field label="Company Name" value={companyForm.name} onChange={v => setCompanyForm(f => ({ ...f, name: v }))} /><Field label="Legal Name" value={companyForm.legal_name} onChange={v => setCompanyForm(f => ({ ...f, legal_name: v }))} /></div>
