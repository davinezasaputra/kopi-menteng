...
class PurchaseOrderApiTest extends TestCase
{
    ...
    {
        $this->tenant = Tenant::create(['name' => 'Test Tenant', 'code' => 'TEST', 'slug' => 'test-tenant']);
        $this->company = Company::create(['tenant_id' => $this->tenant->id, 'code' => 'TEST', 'name' => 'Test Company']);
        $this->branch = Branch::create(['tenant_id' => $this->tenant->id, 'company_id' => $this->company->id, 'code' => 'BDG', 'name' => 'Bandung']);
    ...
    public function test_create_purchase_order_requires_authentication(): void
    {
        $response = $this->postJson('/api/purchasing/orders', [
            ...
            'items' => [['product_id' => $this->product->id, 'quantity' => 10, 'unit_cost' => 100000]],
        ]);
    ...
    {
        $otherTenant = Tenant::create(['name' => 'Other Tenant', 'code' => 'OTHER', 'slug' => 'other-tenant']);
    ...
    public function test_list_purchase_orders_returns_proper_response_shape(): void
    {
        PurchaseOrder::create([
            'tenant_id' => $this->tenant->id, 'company_id' => $this->company->id, 'branch_id' => $this->branch->id,
            'warehouse_id' => $this->warehouse->id, 'supplier_id' => $this->supplier->id, 'order_number' => 'PO-2026-000001',
            'status' => 'draft', 'order_date' => now()->toDateString(), 'subtotal' => 1000000, 'discount_amount' => 0,
            'tax_amount' => 100000, 'grand_total' => 1100000, 'created_by' => $this->user->id,
        ]);

        $response = $this->actingAs($this->user)->getJson('/api/purchasing/orders');
        $response->assertOk()->assertJsonStructure([
            'status', 'data' => ['data' => ['*' => [
                'id','order_number','status','order_date','expected_date','subtotal','discount_amount','tax_amount','grand_total',
                'supplier' => ['id','name','code'], 'warehouse' => ['id','name'],
                'items' => ['*' => ['id','product_id','quantity','unit_cost','line_total']],
            ]]],
        ]);
        foreach (data_get($response->json(), 'data.data.*.supplier') as $supplier) {
    ...
    public function test_order_date_is_business_date_not_utc(): void
    {
        ...
        PurchaseOrder::create([
            'tenant_id' => $this->tenant->id, 'company_id' => $this->company->id, 'branch_id' => $this->branch->id,
            'warehouse_id' => $this->warehouse->id, 'supplier_id' => $this->supplier->id, 'order_number' => 'PO-2026-000002',
            'status' => 'draft', 'order_date' => $businessDate, 'subtotal' => 500000, 'discount_amount' => 0,
            'tax_amount' => 50000, 'grand_total' => 550000, 'created_by' => $this->user->id,
        ]);
        $response = $this->actingAs($this->user)->getJson('/api/purchasing/orders');
        $orderDate = data_get($response->json(), 'data.data.0.order_date');
        $this->assertMatchesRegularExpression('/\d{4}-\d{2}-\d{2}/', $orderDate);
        $this->assertEquals($businessDate, substr($orderDate, 0, 10));
    }

    public function test_purchase_order_calculation_is_correct(): void
    {
        $subtotal = 1000000; $discount = 100000; $tax = 90000; $expectedGrandTotal = $subtotal - $discount + $tax;
        $response = $this->actingAs($this->user)->postJson('/api/purchasing/orders', [
            'supplier_id' => $this->supplier->id, 'warehouse_id' => $this->warehouse->id, 'discount_amount' => $discount,
            'tax_amount' => $tax, 'items' => [['product_id' => $this->product->id, 'quantity' => 10, 'unit_cost' => 100000]],
        ]);
        ...
        $this->assertEquals($expectedGrandTotal, PurchaseOrder::latest()->first()->grand_total);
    }

    public function test_purchase_order_item_line_total_calculated_correctly(): void
    {
        $quantity = 5; $unitCost = 250000; $expectedLineTotal = $quantity * $unitCost;
        $response = $this->actingAs($this->user)->postJson('/api/purchasing/orders', [
    ...
    public function test_purchase_order_starts_in_draft_status(): void
    {
        ...
        $this->assertEquals('draft', PurchaseOrder::latest()->first()->status);
    }
    ...
    public function test_draft_purchase_order_can_transition_to_submitted(): void
    {
        $po = PurchaseOrder::create([
            'tenant_id' => $this->tenant->id, 'company_id' => $this->company->id, 'branch_id' => $this->branch->id,
            'warehouse_id' => $this->warehouse->id, 'supplier_id' => $this->supplier->id, 'order_number' => 'PO-2026-000003',
            'status' => 'draft', 'order_date' => now()->toDateString(), 'subtotal' => 500000, 'discount_amount' => 0,
            'tax_amount' => 50000, 'grand_total' => 550000, 'created_by' => $this->user->id,
        ]);
        PurchaseOrderItem::create(['purchase_order_id' => $po->id, 'product_id' => $this->product->id, 'quantity' => 1, 'unit_cost' => 100000, 'line_total' => 100000]);
        $response = $this->actingAs($this->user)->postJson('/api/purchasing/orders/'.$po->id.'/submit');
        $response->assertOk();
    }
    ...
    public function test_empty_purchase_order_cannot_be_submitted(): void
    {
        $po = PurchaseOrder::create([
            'tenant_id' => $this->tenant->id, 'company_id' => $this->company->id, 'branch_id' => $this->branch->id,
            'warehouse_id' => $this->warehouse->id, 'supplier_id' => $this->supplier->id, 'order_number' => 'PO-2026-000004',
            'status' => 'draft', 'order_date' => now()->toDateString(), 'subtotal' => 0, 'discount_amount' => 0,
            'tax_amount' => 0, 'grand_total' => 0, 'created_by' => $this->user->id,
        ]);
        $response = $this->actingAs($this->user)->postJson('/api/purchasing/orders/'.$po->id.'/submit');
