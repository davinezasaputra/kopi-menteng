...
class Phase1FoundationContractTest extends TestCase
{
    ...
    private function identity(string $role = 'tenant-admin'): array
    {
        ...
        $roleModel=Role::create(['tenant_id'=>$tenant->id,'name'=>ucwords(str_replace('-',' ',$role)),'code'=>$role,'is_system'=>true]);
        $permissions=[
            Permission::create(['module'=>'rbac','resource'=>'role','action'=>'view','name'=>'rbac.role.view']),
            Permission::create(['module'=>'audit','resource'=>'audit_log','action'=>'view','name'=>'audit.audit_log.view']),
        ];
        ...
        Membership::create(['tenant_id'=>$tenant->id,'user_id'=>$user->id,'company_id'=>$company->id,'branch_id'=>$branch->id,'role_id'=>$roleModel->id,'status'=>'active','is_primary'=>true]);
        return [$tenant,$company,$branch,$user,$roleModel];
    }

    public function test_v1_me_exposes_scoped_permissions(): void
    {
        [,,$branch,$user,] = $this->identity();
        $response=$this->actingAs($user,'sanctum')->getJson('/api/v1/me');
        $response->assertOk()->assertJsonPath('data.branch_id',$branch->id);
        $response->assertJsonStructure(['data'=>['tenant_id','company_id','branch_id','role','permissions']]);
    }
    ...
    {
        [,,$,$user,$role]=$this->identity();
        app(TenantContext::class)->setMembership(Membership::where('user_id',$user->id)->firstOrFail());
