...
class ErpMasterSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($users as $definition) {
            $user = User::updateOrCreate(['email'=>$definition['email']], ['name'=>$definition['name'],'password'=>Hash::make(env($definition['password_env'], 'change-me-immediately')),'role'=>$definition['legacy_role'],'pin'=>null]);
            ...
            if (Schema::hasColumn('users','default_tenant_id')) {
                $user->forceFill(['default_tenant_id'=>$tenant->id,'default_company_id'=>$company->id,'default_branch_id'=>$branch->id])->saveQuietly();
            }
        }

        $accounts = [
            ['code'=>'1000','name'=>'Cash','type'=>'asset','normal_balance'=>'debit'], ['code'=>'1010','name'=>'Bank','type'=>'asset','normal_balance'=>'debit'], ['code'=>'1100','name'=>'Inventory Asset','type'=>'asset','normal_balance'=>'debit'], ['code'=>'1200','name'=>'Accounts Receivable','type'=>'asset','normal_balance'=>'debit'], ['code'=>'2100','name'=>'Accounts Payable','type'=>'liability','normal_balance'=>'credit'], ['code'=>'4000','name'=>'Sales Revenue','type'=>'revenue','normal_balance'=>'credit'], ['code'=>'5000','name'=>'Cost of Goods Sold','type'=>'expense','normal_balance'=>'debit'],
        ];
        foreach ($accounts as $account) {
            ErpAccount::updateOrCreate(['tenant_id'=>$tenant->id,'company_id'=>$company->id,'code'=>$account['code']], $account + ['is_postable'=>true,'is_active'=>true]);
        }
        ...
        $products = [
            ['code'=>'KOPI-SUSU','category'=>'Kopi','name'=>'Kopi Susu Gula Aren','price'=>25000,'stock'=>100], ['code'=>'AMERICANO','category'=>'Kopi','name'=>'Americano','price'=>20000,'stock'=>100], ['code'=>'MATCHA','category'=>'Non-Kopi','name'=>'Matcha Latte','price'=>28000,'stock'=>50], ['code'=>'FRIES','category'=>'Makanan / Snack','name'=>'Kentang Goreng','price'=>18000,'stock'=>30],
        ];
        foreach ($products as $product) {
            $query = Product::query()->where('name',$product['name']);
            if (Schema::hasColumn('products','tenant_id')) $query->where('tenant_id',$tenant->id);
            $existing = $query->first();
            $attributes = ['category_id'=>$categoryModels[$product['category']]->id,'name'=>$product['name'],'description'=>'ERP demo master product','price'=>$product['price'],'is_active'=>true];
            if (Schema::hasColumn('products','tenant_id')) $attributes['tenant_id']=$tenant->id;
            ...
            if (Schema::hasTable('inventory_balances')) {
                $exists = DB::table('inventory_balances')->where('tenant_id',$tenant->id)->where('company_id',$company->id)->where('branch_id',$branch->id)->where('warehouse_id',$warehouse->id)->where('product_id',$productModel->id)->exists();
                if (!$exists) DB::table('inventory_balances')->insert(['tenant_id'=>$tenant->id,'company_id'=>$company->id,'branch_id'=>$branch->id,'warehouse_id'=>$warehouse->id,'product_id'=>$productModel->id,'quantity'=>$product['stock'],'reserved_quantity'=>0,'available_quantity'=>$product['stock'],'average_cost'=>round($product['price']*0.40,4),'last_cost'=>round($product['price']*0.40,4),'created_at'=>now(),'updated_at'=>now()]);
            }
        }
        ...
        if (Schema::hasTable('customers')) {
            $customerData = [['name'=>'Customer Retail 001','phone'=>'081300000001','tier'=>'silver','points'=>0], ['name'=>'Customer Gold 001','phone'=>'081300000002','tier'=>'gold','points'=>100]];
            foreach ($customerData as $customer) {
                $query=Customer::query()->where('phone',$customer['phone']);
                if (Schema::hasColumn('customers','tenant_id')) { $query->where('tenant_id',$tenant->id); $customer['tenant_id']=$tenant->id; }
                $query->updateOrCreate(['phone'=>$customer['phone']],$customer);
            }
        }

        if (Schema::hasTable('purchasing_budgets')) PurchasingBudget::updateOrCreate(['tenant_id'=>$tenant->id,'company_id'=>$company->id,'branch_id'=>$branch->id,'budget_year'=>now()->year], ['allocated_amount'=>100000000,'committed_amount'=>0,'spent_amount'=>0,'is_active'=>true,'created_by'=>User::where('email',$users[0]['email'])->value('id'),'updated_by'=>User::where('email',$users[0]['email'])->value('id'),'notes'=>'ERP Demo purchasing budget']);

        if (Schema::hasTable('purchasing_approval_matrix_rules')) {
            $roleId=$roles['purchasing-manager']->id;
            if (!PurchasingApprovalMatrixRule::query()->where('tenant_id',$tenant->id)->where('company_id',$company->id)->where('branch_id',$branch->id)->where('min_amount',0)->where('max_amount',1000000)->exists()) PurchasingApprovalMatrixRule::create(['tenant_id'=>$tenant->id,'company_id'=>$company->id,'branch_id'=>$branch->id,'approver_role_id'=>$roleId,'document_type'=>'purchase_order','min_amount'=>0,'max_amount'=>1000000,'priority'=>1,'is_active'=>true,'notes'=>'Demo purchasing approval up to Rp1 juta']);
            if (!PurchasingApprovalMatrixRule::query()->where('tenant_id',$tenant->id)->where('company_id',$company->id)->where('branch_id',$branch->id)->where('min_amount',1000000)->where('max_amount',10000000)->exists()) PurchasingApprovalMatrixRule::create(['tenant_id'=>$tenant->id,'company_id'=>$company->id,'branch_id'=>$branch->id,'approver_role_id'=>$roleId,'document_type'=>'purchase_order','min_amount'=>1000000,'max_amount'=>10000000,'priority'=>2,'is_active'=>true,'notes'=>'Demo purchasing approval Rp1 juta-Rp10 juta']);
        }

        if (Schema::hasTable('sales_approval_matrix_rules')) {
            $roleId=$roles['sales-manager']->id;
            foreach ([[0,1000000,1,'Demo sales approval up to Rp1 juta'],[1000000,10000000,2,'Demo sales approval Rp1 juta-Rp10 juta']] as [$min,$max,$priority,$notes]) {
                if (!SalesApprovalMatrixRule::query()->where('tenant_id',$tenant->id)->where('company_id',$company->id)->where('branch_id',$branch->id)->where('min_amount',$min)->where('max_amount',$max)->exists()) SalesApprovalMatrixRule::create(['tenant_id'=>$tenant->id,'company_id'=>$company->id,'branch_id'=>$branch->id,'approver_role_id'=>$roleId,'min_amount'=>$min,'max_amount'=>$max,'priority'=>$priority,'is_active'=>true,'notes'=>$notes]);
            }
        }
