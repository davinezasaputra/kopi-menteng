...
class SalesReturnService{
 ...
 public function create(SalesInvoice $invoice,Warehouse $warehouse,array $items,?string $reason=null):SalesReturn{
  return DB::transaction(function()use($invoice,$warehouse,$items,$reason){
   ...
   foreach($normalized as $x){$p=Product::findOrFail($x['product_id']);$cost=(float)(InventoryBalance::query()->where('tenant_id',$i->tenant_id)->where('company_id',$i->company_id)->where('branch_id',$i->branch_id)->where('warehouse_id',$warehouse->id)->where('product_id',$p->id)->value('average_cost')??0);if($cost<=0)$cost=round((float)$p->price*0.4,4);$this->inventory->receive($warehouse,$p,$x['quantity'],$cost,'sales_return',(string)$r->id,$reason);SalesReturnItem::create(['sales_return_id'=>$r->id]+$x);}
   ...
   $this->accounting->postSourceJournal('sales_return',(string)$r->id,'Sales return '.$r->return_number,[['account_id'=>$rev->id,'debit'=>$total,'credit'=>0,'description'=>'Reverse sales revenue'],['account_id'=>$ar->id,'debit'=>0,'credit'=>$total,'description'=>'Reduce accounts receivable']],(int)$r->branch_id,$r->return_date);
   $returnCost=round(array_sum(array_map(function($x)use($i,$warehouse){return (float)$x['quantity']*(float)(InventoryBalance::query()->where('tenant_id',$i->tenant_id)->where('company_id',$i->company_id)->where('branch_id',$i->branch_id)->where('warehouse_id',$warehouse->id)->where('product_id',$x['product_id'])->value('average_cost')??0);},$normalized)),2);if($returnCost>0)$this->accounting->postSourceJournal('sales_return_cogs',(string)$r->id,'Sales return inventory '.$r->return_number,[['account_id'=>$inv->id,'debit'=>$returnCost,'credit'=>0,'description'=>'Returned inventory at cost'],['account_id'=>$cogs->id,'debit'=>0,'credit'=>$returnCost,'description'=>'Reverse COGS']],(int)$r->branch_id,$r->return_date);
   $i->outstanding_amount=max(0,(float)$i->outstanding_amount-min((float)$i->outstanding_amount,$total));$i->status=$i->outstanding_amount<=0.009?'paid':((float)$i->paid_amount>0?'partially_paid':'unpaid');$i->save();$this->audit->record('posted','sales_return',$r,null,$r->toArray());return $r->load(['items.product','invoice','warehouse']);
  });
 }
