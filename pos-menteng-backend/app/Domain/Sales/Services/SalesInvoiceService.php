...
class SalesInvoiceService
{
    ...
    public function createFromShipment(SalesShipment $shipment): SalesInvoice{
        return DB::transaction(function()use($shipment){$m=$this->context->membership();if(!$m)throw ValidationException::withMessages(['context'=>'No active ERP context.']);
            ...
            $this->accounting->postSourceJournal('sales_invoice',(string)$invoice->id,'Sales invoice '.$invoice->invoice_number,[['account_id'=>$ar->id,'debit'=>$total,'credit'=>0,'description'=>'Recognize accounts receivable'],['account_id'=>$revenue->id,'debit'=>0,'credit'=>$total,'description'=>'Recognize sales revenue']],(int)$row->branch_id);
