...
class PayrollAutomationService
{
    ...
    public function autoFillPayroll(Payroll $payroll): array
    {
        ...
        $period = $this->periodStart((string) $payroll->period);
        ...
        $attendanceDeduction = $this->calculateDeductions($employee, $period, $rows, $baseSalary);
        $totalDeduction = max(0, $manualDeduction + $attendanceDeduction);
        $total = max(0, $baseSalary + $allowance - $totalDeduction);
        ...
        $summary = [
            ...
            'absence_days' => $rows->filter(fn ($row) => $this->isAbsenceStatus((string) ($row->status ?? '')))->count(),
    ...
    public function calculateDeductions($employee, Carbon $period, $attendance = null, float $baseSalary = 0): float
    {
        ...
        $absenceDays = $attendance->filter(fn ($row) => $this->isAbsenceStatus((string) ($row->status ?? '')))->count();
        ...
        if ($lateMinutes > 0) {
            $rules = \DB::table('attendance_penalties')
                ...
                ->get();
            $rule = $rules
                ->filter(fn ($row) => $this->thresholdMinutes((string) $row->duration_threshold) <= $lateMinutes)
    ...
    public function generatePayrollPDF(Payroll $payroll): string
    {
        ...
        if (class_exists(\Barryvdh\DomPDF\Facade\Pdf::class)) {
            ...
            Storage::disk('public')->put($path, $this->minimalPdf([
                ...
                'Potongan Manual: Rp ' . number_format((float) $payroll->deduction, 0, ',', '.'),
                'Potongan Attendance: Rp ' . number_format((float) ($payroll->attendance_deduction ?? 0), 0, ',', '.'),
    ...
    public function queuePayrollNotifications(Payroll $payroll, string $pdfPath, ?PayrollAutomationConfig $config = null): array
    {
        ...
        foreach ($recipients as $type => $phone) {
            $normalizedPhone = $phone ? $this->normalizePhone((string) $phone) : '';
            if ($normalizedPhone === '') {
                continue;
            }
    ...
    private function sendViaTwilio(PayrollNotification $notification): array
    {
        ...
        return ['message_id' => $response->json('sid'), 'status' => $response->json('status', 'queued')];
    }
    ...
    private function isAbsenceStatus(string $status): bool
    {
        return in_array($status, ['absence', 'absen', 'alpha', 'tidak_hadir'], true);
    }
