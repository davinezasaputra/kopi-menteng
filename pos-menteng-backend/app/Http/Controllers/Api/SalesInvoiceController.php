...
class SalesInvoiceController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        $data=$request->validate([
            'sales_shipment_id'=>['required','uuid','exists:sales_shipments,id'],
        ]);

        $invoice=$this->service->createFromShipment(
            SalesShipment::findOrFail($data['sales_shipment_id'])
        );

        return response()->json([
            'status'=>'success',
            'message'=>$invoice->wasRecentlyCreated ? 'Sales invoice created.' : 'Existing sales invoice returned.',
            'data'=>$invoice,
