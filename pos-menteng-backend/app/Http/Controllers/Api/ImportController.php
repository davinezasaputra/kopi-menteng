...
class ImportController extends Controller
{
    ...
    public function importRawMaterials(Request $request)
    {
        ...
        try {
            ...
            if($hasMenuHeader)return $this->processMenuImport($rows);
            $import=new RawMaterialImport();
            Excel::import($import,$request->file('file'));
            $failedRows=$import->getFailedRows();
            return response()->json(['status'=>count($failedRows)>0?'warning':'success','message'=>count($failedRows)>0?'Import bahan baku sebagian berhasil.':'Import data bahan baku berhasil.','imported_count'=>$import->getSuccessCount(),'failed_rows'=>$failedRows],200);
        ...
        }catch(\Throwable $e){return response()->json(['status'=>'error','message'=>'Terjadi kesalahan saat mengimport file: '.$e->getMessage()],500);}
    }

    private function processMenuImport($rows)
    {
        $warehouse=Warehouse::query()->where('branch_id',$this->context->branchId())->where('is_default',true)->first()??Warehouse::query()->where('branch_id',$this->context->branchId())->where('status','active')->first();
        if(!$warehouse&&$this->context->branchId()!==null)$warehouse=Warehouse::create(['branch_id'=>$this->context->branchId(),'code'=>'MAIN','name'=>'Main Warehouse','type'=>'store','is_default'=>true,'status'=>'active']);
        ...
        foreach($rows as $index=>$raw){
            $row=$raw instanceof \Illuminate\Support\Collection?$raw:collect((array)$raw);$line=$index+2;
            try{
                $name=trim((string)($row['name']??''));$price=$row['price']??null;$categoryName=trim((string)($row['category']??''));
                ...
                $active=$this->booleanValue($row['is_active']??1);$description=isset($row['description'])?trim((string)$row['description']):null;
                DB::transaction(function()use($name,$price,$categoryName,$active,$description,$warehouse,&$created,&$updated):void{
                    ...
                    InventoryBalance::firstOrCreate(['tenant_id'=>$this->context->tenantId(),'company_id'=>$this->context->companyId(),'branch_id'=>$this->context->branchId(),'warehouse_id'=>$warehouse->id,'product_id'=>$product->id],['quantity'=>0,'reserved_quantity'=>0,'average_cost'=>0]);
                });
